# coding=UTF-8
# 深度强化学习——原理、算法与PyTorch实战，代码名称：代36-PS-DQN算法实验.py

import gym, random, pickle, os.path, math, glob

from atari_py import make_atari, wrap_deepmind
import numpy as np
...
class DQN_PERAgent:
    ...
    def learn_from_experience(self, batch_size):
        if self.memory_buffer.size() > batch_size:
            idxs, states, actions, rewards, next_states, dones = self.sample_from_buffer(batch_size) # 从库采样
            td_loss = self.compute_td_loss(idxs, states, actions, rewards, next_states, dones) # 计算loss
